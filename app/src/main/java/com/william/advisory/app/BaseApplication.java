 ...
 */
package com.william.advisory.app;
...
import com.william.advisory.mvp.model.api.Api;
...
public class BaseApplication extends me.jessyan.art.base.BaseApplication {
    ...
    public void onCreate() {
        ...
        RetrofitUrlManager.getInstance().putDomain(Api.APP_REST_NAME, Api.APP_REST_URL);
        // Log.i("William", "BaseApplication URL :: " + APP_LIVE_URL);

        /*
        RetrofitUrlManager.getInstance().putDomain(APP_DEVELOPMENT_NAME, APP_DEVELOPMENT_URL);
        RetrofitUrlManager.getInstance().putDomain(APP_TEST_NAME, APP_TEST_URL);
        */
...
//        String loginInfoJson = PrefUtils.getInstance(this).getString(Constant.SP_KEY_USER_DATA);
//        if (ValidatorKit.notBlank(loginInfoJson)) {
//            LoginInfo loginInfo = new Gson().fromJson(loginInfoJson, LoginInfo.class);
//            // String sportUrl = loginInfo.getSportUrl();
//            String domainUrl = loginInfo.getDomain();
//
//            //RetrofitUrlManager.getInstance().putDomain(APP_REST_NAME, domainUrl);
//        }
    }
    ...
    private void httpConfig() {
        ...
        //是否debug模式 如果是debug模式打印log
        httpConfig.setDebug(true);
        httpConfig.setTagName("HTTPCaller");//打印log的tagname
