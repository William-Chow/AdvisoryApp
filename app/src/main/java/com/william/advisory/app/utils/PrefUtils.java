...
/**
 * Created by William Chow on 2019-04-18.
 */
public final class PrefUtils {
    ...
    public static void storeUserProfile(Application _mApplication, UserProfile _userProfile) {
        AppComponent appComponent = ((App) _mApplication).getAppComponent();
        PrefUtils.getInstance(_mApplication).put(Utils.SP_KEY_USER_DATA, appComponent.gson().toJson(_userProfile));
    }
     ...
     */
    public void logout() {
        remove(Utils.SP_KEY_USER_DATA);
    }

    UserProfile getUserProfile() {
        String user_json = getString(Utils.SP_KEY_USER_DATA);
        if (Utils.notBlank(user_json)) {
            return new Gson().fromJson(user_json, UserProfile.class);
        }
        return null;
    }
}
